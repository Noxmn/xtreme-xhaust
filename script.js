...
/* Full rebuild script: faster fade, preload checks, correct filenames */
...
function preload(src){ return new Promise((res, rej) => { const i = new Image(); i.onload = () => res(i); i.onerror = () => rej(new Error('Missing file: ' + src)); i.src = src; }); }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  let carIndex = 0, imgIndex = 0, autoTimer = null, progressTimer = null;
  ...
  function startProgress(){
    ...
    progressTimer = setInterval(()=>{ w+=inc; fill.style.width = w + '%'; if (w>=100){ clearInterval(progressTimer); progressTimer=null; } }, interval);
  }

  function resetAuto(){ clearTimeout(autoTimer); autoTimer = setTimeout(nextImage, 5000); }
  ...
  async function loadImage(){
    ...
    titleEl = document.createElement('div'); titleEl.className='story-title'; titleEl.textContent = car.car; titleEl.style.opacity = '1';
    ...
    imgEl = document.createElement('img'); imgEl.className='story-image'; imgEl.src = src; imgEl.alt = car.alt || car.car; imgEl.style.opacity = '0';
    storyContainer.appendChild(imgEl);

    // faster fade: 300ms controlled via CSS variable
    setTimeout(()=>{ if (titleEl) titleEl.style.opacity='0'; if (imgEl) imgEl.style.opacity='1'; }, 200);
  ...
  function nextImage(){
    const car = portfolioData[carIndex];
    if (imgIndex < car.images.length - 1){ imgIndex++; loadImage(); }
    else if (carIndex < portfolioData.length - 1){ carIndex++; imgIndex=0; loadImage(); }
    else closePortfolio();
  }
  function prevImage(){
  ...
  function openPortfolio(start=0){
    if (!portfolioData.length){ alert('No portfolio images found. Make sure portfolio.json and images are in the same folder as index.html'); return; }
  ...
  function closePortfolio(){
    ...
    if (titleEl) { titleEl.remove(); titleEl=null; } if (imgEl) { imgEl.remove(); imgEl=null; }
  }
  ...
  document.addEventListener('keydown', (e)=>{
    if (portfolioStory.style.display==='block'){ if (e.key==='ArrowRight' || e.key===' ') nextImage(); else if (e.key==='ArrowLeft') prevImage(); else if (e.key==='Escape') closePortfolio(); }
  });

  let startX=0,endX=0;
  document.addEventListener('touchstart',(e)=>{ if (portfolioStory.style.display==='block'){ startX = e.changedTouches[0].screenX; clearTimeout(autoTimer); clearInterval(progressTimer); } }, {passive:true});
  document.addEventListener('touchend',(e)=>{ if (portfolioStory.style.display==='block'){ endX = e.changedTouches[0].screenX; if (endX < startX - 50) nextImage(); else if (endX > startX + 50) prevImage(); else { resetAuto(); startProgress(); } } }, {passive:true});
  ...
  window.addEventListener('resize', updateSafeArea); updateSafeArea();
});
